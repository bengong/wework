...
public class MediaMethods extends AbstractMethods {	
	 ...
	 * @return
	 */
	public Object upload(String type, Map<String, Object> data) {		
		// type 媒体文件类型，分别有图片（image）、语音（voice）、视频（video），普通文件（file）
		String url =  String.format("/media/upload?access_token=%s&type=%s", gettoken(), type);
	 ...
	 * @return
	 */
	public Object uploadimg(Map<String, Object> data) {
		String url =  String.format("/media/upload?uploadimg=%s", gettoken());		
		return upload(url, data, Header.create());
	}
	 ...
	 * @return
	 */
	public Object downloadVoice(String media_id) {
		String url =  String.format("/media/get/jssdk?access_token=%s&media_id=%s", gettoken(), media_id);		
		return download(url);
	}
}
